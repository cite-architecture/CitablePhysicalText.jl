...
function parsedatamodel(cexsrc::AbstractString; delimiter = "|")
    ...
    datapairs = reblock(cexsrc, delimiter = delimiter)
    @warn("Number datapairs", length(datapairs))

    
    for pr in datapairs
        if pr[1] in codexcollections
            println("Woot! $(pr[2])")
        else
            println("Sad.")
        end
...
function reblock(cexsrc; delimiter = "|")
    ...
    datapairs = Tuple{CitableObject.Cite2Urn, Any}[]
